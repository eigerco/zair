...
/// Pre-computed message hashes for signing: shared hash and/or per‑proof hashes.
///
/// If both are provided, per‑proof hashes override the shared hash for matching nullifiers.
/// At least one of `shared` or `per_proof` must be non‑empty for each proof that is signed.
#[derive(Debug, Clone, Default)]
pub struct MessageHashes {
    /// Shared message hash applied to all proofs (unless overridden by `per_proof`).
    pub shared: Option<[u8; 32]>,
    /// Per‑proof message hashes keyed by airdrop nullifier.
    pub per_proof: BTreeMap<Nullifier, [u8; 32]>,
}

impl MessageHashes {
    /// Create `MessageHashes` with a single shared message hash.
    #[must_use]
    pub const fn shared(hash: [u8; 32]) -> Self {
        Self {
            shared: Some(hash),
            per_proof: BTreeMap::new(),
        }
    }

    /// Create `MessageHashes` with per‑proof message hashes only.
    #[must_use]
    pub const fn per_proof(map: BTreeMap<Nullifier, [u8; 32]>) -> Self {
        Self {
            shared: None,
            per_proof: map,
        }
    ...
    /// Returns `true` if no messages are provided (both `shared` is `None` and `per_proof` is
    /// empty).
    #[must_use]
    pub fn is_empty(&self) -> bool {
        self.shared.is_none() && self.per_proof.is_empty()
    }
}
...
/// * `message_hashes` - pre-computed message hashes to sign; can be a shared hash, per‑proof
///   hashes, or both (mixed mode)
///
/// # Returns
///
...
pub async fn sign_claim_submission_from_bytes(
    ...
    let shared_hash = message_hashes.shared;
    let per_proof_hashes = message_hashes.per_proof;
...
mod tests {
    ...
    #[test]
    fn message_hashes_is_empty() {
        let input = MessageHashes::default();
        assert!(input.is_empty());
    }

    #[test]
    fn message_hashes_shared_not_empty() {
        let hash = hash_message(b"hello");
        let input = MessageHashes::shared(hash);
        assert!(!input.is_empty());
        assert_eq!(input.shared, Some(hash));
    }

    #[test]
    fn message_hashes_per_proof_not_empty() {
        let nullifier = Nullifier::default();
        let mut map = BTreeMap::new();
        let hash = hash_message(b"world");
        map.insert(nullifier, hash);
        let input = MessageHashes::per_proof(map);
        assert!(!input.is_empty());
        assert!(input.shared.is_none());
    }
}
