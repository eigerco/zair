...
pub enum ApiError {
    ...
    ///
    /// Provide a shared hash via `MessageHashes::shared()` or per-proof hashes via
    /// `MessageHashes::per_proof()`, or both (per-proof takes precedence over shared).
    #[error(
        "No message hashes provided for signing. Use MessageHashes::shared(hash) or MessageHashes::per_proof(map)"
    )]
...
/// * `message_hashes` - optional pre-computed message hashes to sign; can be a shared hash,
///   per‑proof hashes, or both (mixed mode)
///
/// # Errors
///
...
pub async fn run(
    ...
    message_hashes: Option<MessageHashes>,
